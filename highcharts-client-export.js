((H => {
  ...
  const oldExport = H.Chart.prototype.exportChart;
  H.Chart.prototype.exportChart = () => {};

  // Set the URL of the export server to a non-existant one, just to be sure.
  const defaultHighChartsOptions = H.getOptions() || {};
  ...
  const TRANSLATION_KEY_TO_MIME_TYPES = {
  ...
  let browserSupportDownload = false;
  ...
  const browserSupportBlob = window.Blob && window.navigator.msSaveOrOpenBlob;
  ...
  H.exporting.USE_TITLE_FOR_FILENAME = function (options, chartOptions) {
    const title = this.title ? this.title.textStr.replace(/ /g, '-').toLowerCase() : 'chart';
    return title;
  };

  const supportStatus = {};
  const buildSupportStatus = () => {
    const hasDownloadOrBlob = browserSupportDownload || browserSupportBlob;
    ...
    const svgSupport = (H.Chart.prototype.getSVG !== undefined);
    ...
    const canvgSupport = typeof canvg !== 'undefined' && typeof RGBColor != 'undefined' &&
  ...
  const handlerBuilder = mimeType => function () {
   ...
   */
  const svgToCanvas = (svg, width, height, callback) => {
    const canvas = document.createElement('canvas');
    ...
    canvg(canvas, svg, {
      ...
      scaleWidth: width,
      scaleHeight: height,
      renderCallback() { callback(canvas); },
    });
   ...
   */
  class Opt {
    constructor(opts1, opt2, dotdotdot) {
  ...
  const preRenderCsvXls = (highChartsObject, options, chartOptions) => {
    const opt = new Opt(options, highChartsObject.options.exporting, defaultExportOptions);

    // Copies some values from the options, so we can set it and change those
    // through the options argument.
    const hasCSVOptions = highChartsObject.options.exporting && highChartsObject.options.exporting.csv;
    const csvOpt = new Opt((options || {}).csv, (highChartsObject.options.exporting || {}).csv, defaultExportOptions.csv);

    const oldOptions = {};
    const optionsToCopy = ['dateFormat', 'itemDelimiter', 'lineDelimiter'];
    let optionToCopy;
    for (const i in optionsToCopy) {
  ...
  const renderCsv = (highChartsObject, context, callback) => {
      const data = {
          content: undefined,
          datauri: undefined,
          blob: undefined,
      ...
      const csv = (context.useBOM ? UTF8_BOM : '') + highChartsObject.getCSV(context.useLocalDecimalPoint);
      data.content = csv;

      callback(data);
    };

  const renderXls = (highChartsObject, context, callback) => {
      const data = {
          content: undefined,
          datauri: undefined,
          blob: undefined,
      ...
      const xls = `${context.useBOM ? UTF8_BOM : ''}<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel"xmlns="http://www.w3.org/TR/REC-html40"><head><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>Sheet</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--><style>td{border:none;font-family: Calibri, sans-serif;} .number{mso-number-format:"0.00";}</style><meta name=ProgId content=Excel.Sheet></head><body>${highChartsObject.getTable(context.useLocalDecimalPoint)}</body></html>`;
      data.content = xls;

      callback(data);
    };

  const postRenderCsvXls = (highChartsObject, context) => {
      if (context.hasCSVOptions) {
        for (const i in context.optionsToCopy) {
  ...
  const preRenderImage = (highChartsObject, options, chartOptions) => {
    const opt = new Opt(options, highChartsObject.options.exporting, defaultExportOptions);

    const scale = opt.get('scale');
    const sourceWidth = highChartsObject.options.width || opt.get('sourceWidth') || highChartsObject.chartWidth;
    const sourceHeight = highChartsObject.options.height || opt.get('sourceHeight') || highChartsObject.chartHeight;
    const destWidth = sourceWidth * scale;
    const destHeight = sourceHeight * scale;

    const cChartOptions = chartOptions || highChartsObject.options.exporting && highChartsObject.options.exporting.chartOptions || {};
    ...
    const svg = highChartsObject.getSVG(cChartOptions);
  ...
  const renderSvg = (highChartsObject, context, callback) => {
      const data = {
          content: undefined,
          datauri: undefined,
          blob: undefined,
  ...
  const renderPngJpeg = (highChartsObject, context, callback) => {
      const data = {
          content: undefined,
          datauri: undefined,
          blob: undefined,
      ...
      svgToCanvas(context.svg, context.destWidth, context.destHeight, canvas => {
  ...
  const renderPdf = (highChartsObject, context, callback) => {
      const data = {
          content: undefined,
          datauri: undefined,
          blob: undefined,
      ...
      svgToCanvas(context.svg, context.destWidth, context.destHeight, canvas => {
        const doc = new jsPDF('l', 'mm', [context.destWidth, context.destHeight]);;
  ...
  const download = (highChartsObject, context, data) => {
  ...
  H.Chart.prototype.exportChartLocal = function (options, chartOptions) {
    const opt = new Opt(options, this.options.exporting, defaultExportOptions);
    ...
    const filename = opt.get('filename');
    ...
    steps.rendering[type].render(highChartsObject, context, data => {
